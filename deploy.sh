...
WGET_URL=http://localhost/${CONTEXT_NAME}/html/index.html
if [ "${enable_SSI}" = "on" ]; then
  echo --- exec wget %WGET_URL% ---
  sudo rm -rf "${GEN_PATH}in/wget"
  sudo wget -x -r --level=10 ${WGET_URL} > /dev/null 2>&1 -P "${GEN_PATH}in/wget"
  echo --- copy html and web to in directory ---
  cp -aR "${GEN_PATH}in/wget/localhost/${CONTEXT_NAME}/html" "${GEN_PATH}in/html"
  cp -aR "${SITE_PATH}web" "${GEN_PATH}in/web"
  cp -aR "${GEN_PATH}in/wget/localhost/${CONTEXT_NAME}/web" "${GEN_PATH}in/"
else
  echo --- copy html and web to in directory ---
  cp -aR "${SITE_PATH}html" "${GEN_PATH}in/html"
  cp -aR "${SITE_PATH}web" "${GEN_PATH}in/"
fi
...
cp -aR "${SITE_PATH}db/mapper" "${GEN_PATH}in/"
...
#rm -rf "${PRJ_PATH}"
#cp -aR "${WAO_PATH}project/${BLANK_PRJ}" "${PRJ_PATH}"  #(eclipse 消したら　復活)
...
MAPPER_PATH=src/${root_package//./\/}/

cp -aR "${GEN_PATH}in/mapper" "${PRJ_PATH}/${MAPPER_PATH}"

echo --- change path ---   #change path 追加
# 一時変更-------------------------------------------------------
sed -i -e "s/\.\.\/web\/img/img/g" "${GEN_PATH}in/web/css/style.css"
find "${GEN_PATH}out/web/jsp/" -name "*.jsp" | xargs sed -i -e "s/\.\.\/web\///g"
# 一時変更-------------------------------------------------------
...
echo --- copy dat file ---
sudo rm -rf "${webapps_path}/${CONTEXT_NAME}/webapps/dat"
sudo mkdir -p "${webapps_path}/${CONTEXT_NAME}/webapps/dat"
sudo cp -aR "${RELEASE_PATH}/${DAT_FILE_NAME}" "${webapps_path}/${CONTEXT_NAME}/webapps/dat"
